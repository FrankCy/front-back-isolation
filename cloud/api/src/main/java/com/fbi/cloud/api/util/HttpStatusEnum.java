package com.fbi.cloud.api.util;
...
public enum HttpStatusEnum {
    ...
    // I_AM_A_TEAPOT(4000418, "I'm a teapot", ""), 该代码没有被服务器实现

    // INSUFFICIENT_SPACE_ON_RESOURCE(4000419, "Insufficient Space On Resource", ""),  已经过时

    // METHOD_FAILURE(4000420, "Method Failure", ""),  已经过时

    // DESTINATION_LOCKED(4000421, "Destination Locked", ""),  已经过时
    ...
    PARAMETER_ERROR(4001001, "parameter error", "参数异常"),
    PARAMETERS_CANNOT_BE_EMPTY(4001001, "parameters cannot be empty", "参数不能为空"),
    INSUFFICIENT_AUTHORITY(4001002, "Insufficient authority", "权限不足"),
    LOGIN_FAILURE(4001003, "Login Failure", "登陆失效，请重新登陆！"),
    USER_ACCOUNT_EXPIRED(4001004, "user account expired", "账户过期"),
    USER_CREDENTIALS_ERROR(4001005, "user credentials error", "密码错误"),
    USER_CREDENTIALS_EXPIRED(4001006, "user credentials expired", "密码过期"),
    USER_ACCOUNT_DISABLE(4001007, "user account disable", "账号不可用"),
    USER_ACCOUNT_LOCKED(4001008, "user account locked", "账号锁定"),
    USER_ACCOUNT_NOT_EXIST(4001009, "user account not exist", "用户不存在"),
    COMMON_FAIL(4001010, "common fail", "验证发生不可预期的错误"),
    FIND_FAIL(4001011, "find fail", "查询错误");
